...
export class UserService{
  ...
  async create(input:CreateUserInput):Promise<UserOutput>{
    let existingUser = await this.userRepository.findByUsername(input.username)
    if(existingUser){
    ...
    const user = await this.userRepository.create(inputWithHashedPassword)
  ...
  async updateUserById(userId: number, input: UpdateUserInput):Promise<UserOutput>{
    ...
    const user = await this.userRepository.updateUserById(userId,inputWithHashedPassword)
    if(!user){
      throw new UserNotFoundError()
    }
    return {
