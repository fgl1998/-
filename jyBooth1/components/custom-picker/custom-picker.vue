<template>
	...
	<picker :mode="mode" :fields="fields" @change="PickerMode" @cancel="pickerCancel" name="mode" :disabled="!isEdit"
		:value="rangeIndex" range-key="name" :range="data">
		...
		methods: {
			init() {
				...
				if (this.collect) { // collect存在 查码表
					...
					if (codeCacheMap && codeCacheMap[this.collect]) {
						// 先过滤 在赋值
						let temp = codeCacheMap[this.collect];
						if (this.filterCode.length > 0) {
							if (this.reverseFilter) {
								temp = temp.filter((item, index) => {
									return this.filterCode.indexOf(item.code) >= 0
								})
							} else {
								temp = temp.filter((item, index) => {
									return this.filterCode.indexOf(item.code) < 0
						...
						this.$utils.appStorage(this, (res) => {
							if (res[this.collect]) {
								let temp = res[this.collect];
								if (this.filterCode.length > 0) {
									if (this.reverseFilter) {
										temp = temp.filter((item, index) => {
											return this.filterCode.indexOf(item.code) >= 0
