...
<template>
  ...
  >
    <div class="positionLibrary">
          ...
          <div class="info_box">
            ...
            <span class="salary"
              >{{ item.acb21h ? getAbbreviatSalary(item.acb21h) : '未知最低月薪' }}-{{
                item.acb21j ? getAbbreviatSalary(item.acb21j) : '未知最高月薪'
              }}</span
            >
          </div>
          <div class="time">
      ...
      <div class="btns">
        <el-button type="primary" @click="handleApplicate">提交</el-button>
...
export default {
  ...
  methods: {
    ...
    initJobList(aab001, acb330) {
      ...
      this.$http.post(this.$requestConstant.businessRequestType, 'jy204_hrm210', null, params, true).then((data) => {
    ...
    handleChoseJob(jobInfo) {
      let isExit = this.choseJobList.find((item) => item.acb210 == jobInfo.acb210)
      if (isExit) {
        this.choseJobList = this.choseJobList.filter((item) => item.acb210 != jobInfo.acb210)
        this.$delete(jobInfo, 'isChose')
      } else {
        this.choseJobList.push(jobInfo)
        this.$set(jobInfo, 'isChose', true)
      }
    },
    // 关闭弹窗
    handleClose(done) {
    ...
    handleApplicate() {
      ...
      this.addThisJobFair.acb481 = '2'
      let param = {
        cb21ListJson: JSON.stringify(this.choseJobList),
        ...this.addThisJobFair, // 招聘会信息
        yae100: 19,
        acb340: this.userInfo.acb340,
      }
      delete param.aac147
      this.$http.post(this.$requestConstant.businessRequestType, 'jy204_hrm205', null, param, true).then((data) => {
        this.$utils.showToast('已选职位参会成功', 'success')
        this.handleClose()
      })
    },
  },
