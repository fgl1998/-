...
function send() {
  setInterval(() => {
    console.log(22222222222, window.location.href)

    let page = window.location.href
    let currentPage = page.split('#')[1]
    let deviceId = uni.getStorageSync(
      currentPage == bigScreenUrl || currentPage == recruitmentAreaUrl ? 'deviceInfo_2' : 'deviceInfo_1',
    ).ace711
    let ace772 = uni.getStorageSync(
      currentPage == bigScreenUrl || currentPage == recruitmentAreaUrl ? 'deviceInfo_2' : 'deviceInfo_1',
    ).ace772
    if (deviceId && ace772) {
      let param = {
      ...
      app_request.post(requestUrl.commonRequestType, 'jy204_hrm259', null, param, false).then((res) => {
        ...
        if (res?.data?.result?.code == '-1001') {
          ...
          } else if (currentPage == recruitmentAreaUrl) {
            uni.removeStorageSync('deviceInfo_2')
            uni.reLaunch({
              url: recruitmentAreaUrl,
            })
          } else {
