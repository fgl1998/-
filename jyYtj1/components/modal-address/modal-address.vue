  ...
  methods: {
    ...
    handleGetAddress(_item = {}, num) {
      let param = {
        method: 'jy007_com01',
        root: _item.areaid,
        areacode: _item.areaid, //山西省
      }
      this.$utils.request(null, param, true).then((res) => {
        if (res.taareaList.length > 0) {
          let _leaf = true
          for (let items of res.taareaList) {
            if (!items.leaf) {
              _leaf = items.leaf
              break
            }
          ...
          if (_leaf) {
            this.addressLastData = res.taareaList
          } else {
            if (num == '1') {
              this.addressSecondData = res.taareaList
            } else if (num == '2') {
              this.addressTirdData = res.taareaList
            } else if (num == '3') {
              this.addressFourData = res.taareaList
            } else {
              this.addressLastData = res.taareaList
            }
          }
    ...
    changeType: function (item, num) {
      ...
      const result = item.namepath.replace(/^山西省\//, '')
      this.checkText = result

      this.handleGetAddress(item, num)
      this.checkSelect = {
        areaCode: item.areaid,
        // areaname: item.namepath,
        areaname: this.checkText,
      }
    },
