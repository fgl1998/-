						...
						<div class="screen-term">
									...
									<div class="sitem-box">
										<div class="tags-box">
											<span :class="{'tag-item':true,'active': parseFloat(acb117Index) == index}"
												...
												@click="changeAcb117(item,index)">{{item.label}}</span>
										</div>
									</div>
								</div>
								<div class="sitem-group">
							...
							<div class="screen-item" v-else>
								<div class="sitem-group">
									...
									<div class="sitem-box">
										<input class="uni-input" v-model="aab004" placeholder="请输入单位名称" />
										<!-- 	<input class="uni-input" v-model="unitName" placeholder="请输入单位名称" id="initUnit"/> -->
										<!-- <common-keyboard :visibleKeyboard='visibleKeyboard' @updatekey="(val)=>{unitName = val}"></common-keyboard> -->
										<!-- 	<common-keyboard placeholder="请输入单位名称" @getSearchValue="getSearchValue" idName='inputUnit' ></common-keyboard> -->

										<!-- <common-keyboard placeholder="请输入单位名称" :classStyle="classStyle" :key="key" :record="recordName" @getSearchValue="(val)=>{ aab004 = val}" idName='inputUnit' > </common-keyboard> -->
								...
								<div class="sitem-group">
									...
									<div class="sitem-box">
										<input class="uni-input" v-model="ab01Place" placeholder="请输入招聘地址" />
										<!-- <common-keyboard placeholder="请输入招聘地址" :classStyle="classStyle" :key="key" :record="recordAddress" @getSearchValue="(val)=>{ ab01Place = val}" idName='inputAddress' > </common-keyboard> -->

									</div>
								</div>
								<div class="sitem-group">
									<div class="sitem-title">职位名称</div>
									<div class="sitem-box">
										<!-- <input class="uni-input" v-model="acb213" placeholder="请输入岗位名称" /> -->
										<custom-data-picker showValueType="last" v-model="aab022" @change="changeJob"
											:treeData="workTypeTree" labelKey="aca112" valueKey="aca111">
										</custom-data-picker>
										<!-- <custom-picker :range='AuditOrgList' v-model='acb213'> -->
										<!-- </custom-picker> -->
										<!-- <common-keyboard placeholder="请输入岗位名称" :classStyle="classStyle" :key="key" :record="recordJob" @getSearchValue="(val)=>{ acb213 = val}" idName='inputJob' > </common-keyboard> -->
									</div>
								</div>
							</div>
						</div>
	...
	export default {
		...
		data() {
			return {
				...
				acb117: '',
				acb117Index: '',
				acb117List: uni.getStorageSync('codeCacheMap')['ACB214'],
		...
		methods: {
			...
			changeType: function(type) {
				...
				this.recordName = this.aab004
				this.recordAddress = this.ab01Place
				this.recordJob = this.acb213
			},
			...
			changeAcb117: function(item, index) {
				if (!isNaN(parseFloat(this.acb117Index)) && index == parseFloat(this.acb117Index)) {
					...
					this.acb117 = item.value //this.acb117Arr[index]
				}
			},
			changeJob(value){
				this.aab022Index = '';
				...
				this.aab022Arr.forEach((item,index) => {
					if(item.aaa102 == data.value) {
						this.aab022Index = index
						this.aab022_dsc = data.text
					}
				})
			},
