      ...
      <div class="group-con">
        <div class="group-list">
          <div class="group-head">
            ...
            <div class="group-th">担任职位</div>
            <div class="group-th">开始时间</div>
            <div class="group-th">结束时间</div>
            <div class="group-th">项目描述</div>
          ...
          <div class="group-body no-br">
            <div class="group-tr" v-for="(item,index) in cc28List" :key="index">
              <div class="group-td">{{item.acc321}}</div>
              <div class="group-td">{{item.acc322}}</div>
              <div class="group-td">{{$utils.dateFormat(item.aae030)}}</div>
              <div class="group-td">{{$utils.dateFormat(item.aae031)}}</div>
              <div class="group-td">{{item.acc323}}</div>
          ...
          <div class="group-form" v-if="isAdd||isEdit">
            <uni-forms ref="form" :modeValue="formData">
              <div class="form-tr">
                <div class="form-td">
                  <uni-easyinput type="text" v-model="formData.acc321" :clearable="false" placeholder="请输入项目名称" />
                </div>
                <div class="form-td">
                  <uni-easyinput type="text" v-model="formData.acc322" :clearable="false" placeholder="请输入担任职位" />
                </div>
                <div class="form-td">
              ...
              <div class="form-tr">
                <div class="form-td">
                  <uni-easyinput type="text" v-model="formData.acc323" :clearable="false" placeholder="请将输入项目简介" />
                </div>
              </div>
            </uni-forms>
            <div class="group-add">
    ...
    methods: {
      fnAdd() {
        if (this.cc28List.length < 5) {
          this.isAdd = true
		  this.isEdit = false
		  this.formData.acc320 = ''
        } else {
          this.$utils.showToast("最多五条项目，请先删除不需要的")
        }
      },
      //删除
      fnDel(item) {
        this.$emit("delCcList", '5', item.acc320)
      },
	  // 编辑
	  fbEdit(item){
	  	this.isEdit = true
      ...
      saveInfo() {
        if (!this.formData.acc321) {
          this.$utils.showToast("请输入项目名称")
          return
        }
        if (!this.formData.acc322) {
          this.$utils.showToast("请输入担任职务")
          return
        }
        if (!this.formData.acc323) {
          this.$utils.showToast("请输入项目描述")
          return
        }
