                    ...
                    <ul class="listPoint">
                      ...
                      >
                        ...
                        <div>
                          ...
                          <div v-if="item.acb21g || item.aac011" class="info-grid">
                            <div class="info-cell">
                              招聘人数：<span>{{ item.acb21g || '0' }}</span>
                            </div>
                            <div class="info-cell">
                              已投递简历：<span>{{ item.ace755 || '0' }}</span>
                            </div>
                            <div class="info-cell">
                              学历：<span>{{ item.aac011 ? $utils.queryCodeDesc('aac011', item.aac011) : '--' }}</span>
                            </div>
                            <div class="info-cell">
                              经验：<span>{{ item.aac027 || '--' }}</span>
                            </div>
                          </div>
                        </div>
                      </li>
...
export default {
  ...
  methods: {
    ...
    getXzqhMsgList(xzqhVal, skipAdcode, forceMarker) {
      ...
      return this.$utils.request(null, param, true).then(async (res) => {
        ...
        const currentAdcode6 = (xzqhVal || '').toString().slice(0, 6)

        // 市级点击或强制 marker：直接渲染点位，不再下钻区县气泡
        if ((adcodeLevel === 'city' && skipAdcode && skipAdcode === currentAdcode6) || forceMarker) {
          if (this.cityCircles.length && this.map) {
            this.cityCircles.forEach((ov) => this.map.removeOverLay(ov))
            this.cityCircles = []
          }
          const center = this.renderMarkers(markerList, currentAdcode6)
          if (center) {
    ...
    async drawCityCircles(list, skipAdcode) {
      ...
      const drawCircle = (center, total, adcode, item, nameText) => {
        ...
        const clickHandler = (evt) => {
          ...
          // 点击圈层：直接渲染当前市的 marker（不再绘制区县气泡）
          if (adcode) {
            this.getXzqhMsgList(`${adcode}000000`.slice(0, 12), adcode, true).then((nextCenter) => {
              const target = nextCenter || fixedCenter
              this.map.centerAndZoom(target, 12)
            })
          }
        }
...
#mapAll ::v-deep .tdt-infowindow-content {
  width: 303px !important;
  border-radius: 4px 0px 0px 4px;
  margin: 0;
}
#mapAll ::v-deep .tdt-infowindow-content-wrapper {
