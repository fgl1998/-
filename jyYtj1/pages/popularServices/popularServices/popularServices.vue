	...
	<div class="page">
		...
		<div class="page-wrap">
			<div class="wrap-center">
				...
				<div class="job-type">
					<div :class="['type-item','type-bg'+index]" v-for="(item,index) in jobType" :key="index"
						@click="toPage(item)">
				...
				<div class="job-list">
					<div class="list-title">
						...
						<div class="title-rt">
							<button class="btn btn-refresh" @click="changeQuery"><i
									class="ic ic-refresh"></i><span>换一批</span></button>
						</div>
					</div>
					<div class="list-con">
							...
							<div class="item-top">
								...
								<div class="item-info">
									...
									<div class="info-money">
										<!-- <span>{{item.acb21h+'-'+item.acb21j}}</span>元/月 -->
										...
										<label
											style="padding-left: 6px;">{{item.acb21h||item.acb21j ? '元/月' : ''}}</label>
									</div>
								</div>
							</div>
	...
	export default {
		...
		data() {
			return {
				...
				jobType: [
					// 	{
					// 	logo: '../../../static/images/popular_nav0.png',
					// 	name: '零工市场',
					// 	url: '/pages/jobRecruitment/findWork/findWork?acb211=3'
					// }, 
					{
						logo: '../../../static/images/popular_nav1.png',
						name: '社会招聘',
						url: '/pages/jobRecruitment/findWork/findWork?acb211=1'
					}, {
						logo: '../../../static/images/popular_nav2.png',
						name: '校园招聘',
						url: '/pages/jobRecruitment/findWork/findWork?acb211=2'
					},
					{
						logo: '../../../static/images/archives_ytj.png',
						name: '档案服务',
						url: '/pages/archivesQuery/archivesQuery'
					},
				],
				jobList: [],
				currentPage: 1,
			...
			init() {
				...
				this.$utils.request('', param, true).then(res => {
					if (res.serviceSuccess) {
						...
						if (this.currentPage > res.data.pageBean.pageNum) {
							this.$utils.showToast("暂无更多推荐的岗位信息")
						}
					} else {
						this.$utils.showToast("查询推荐的岗位失败")
					}
				})
			},
			toPage(item) {
