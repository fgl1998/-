          ...
          <div class="cell-items">
            ...
            <block>
              <ComponentDeclarePicker
                ...
                placeholder="请选择是否开校直接免收学费"
                v-model="projectDeclareInfo.j00088"></ComponentDeclarePicker>
            </block>
            <!-- 同时申报 -->
            <block v-if="!isEditDeclareInfo">
              <ComponentDeclareRadio
                label="是否同时申报“普通高中国家助学金”"
                type="radio"
                :range="codeMap.yesorno"
                v-model="projectDeclareInfo.chb0g5"></ComponentDeclareRadio>
            </block>
          </div>
        </div>
      </ComponentPanel>
...
<script>
  ...
  import { getDate, getNumInString, getAge } from '@/utils/get'
    ...
    methods: {
      ...
      setDeclareInfo(data, e) {
        ...
        const {
          ...
          chb0g4, // 是否同时申报“普通高中国家助学金”
          chb0g5, // 是否同时申报“免除普通高中家庭经济困难学生学费”
          chb0g3 // 家庭人均收入
        } = hb00BizMap
        this.aka131 = aka131
        // ===== 基本信息设置 start ======
      ...
      handlePickerChange(id, val_desc, val) {
        if (id === 'j00000') {
          ...
          // 市本级
          if (val === '165015') {
            this.chi031 = '20015' // 申报项目编码
            this.chi031_desc = '市本级免除普通高中家庭经济困难学生学费' // 申报项目中文
          } else {
            // 县级
            this.chi031 = '20016' // 申报项目编码
            this.chi031_desc = '县级免除普通高中家庭经济困难学生学费' // 申报项目中文
          }
        }
        if (id === 'j00001') {
          this.$store.commit(M_UPDATE_USER_LIST, {
      ...
      handleCheckInput() {
        ...
        if (!chb0g5) {
          showModal('请选择是否同时申报“免除普通高中家庭经济困难学生费”')
          return false
        }
        if (!j00088) {
          showModal('请选择是否保教费')
      ...
      handleJumpNextStep() {
        ...
        const hb00BizMap = {
          ...
          aac005: nation, // 民族
          j00088, //是否报教费
          chb0g4: chb0g5, // 是否同时申报“普通高中国家助学金”
          chb0g5, // 是否同时申报“免除普通高中家庭经济困难学生学费”
          chb0g3: getChb0g3, // 家庭人均收入
        ...
        if (chb0g5 === '1') {
          // 市本级
          if (j00000 === '165015') {
            this.$store.commit(UPDATE_OTHERS_DECLARE_INFO, {
              chi031: '20014',
              chi031_desc: '市本级普通高中国家助学金'
            })
          } else {
            // 县级
            this.$store.commit(UPDATE_OTHERS_DECLARE_INFO, {
              chi031: '20013',
              chi031_desc: '县级普通高中国家助学金'
            })
          }
        }
    ...
    watch: {
      'projectDeclareInfo.j00011desc': {
        handler(val) {
          let str = '暂无'
          if (val) {
            const result = getNumInString(val)
            if (result.length > 0) {
              str = result[0] + '年9月'
            }
          }
