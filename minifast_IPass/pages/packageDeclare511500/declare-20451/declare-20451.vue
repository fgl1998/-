...
export default {
  ...
  methods: {
    ...
    async getCodeTableCacheData() {
      ...
      try {
        m00100 = await getCodeListByCodeName('m00100')
      } catch (err) {
        console.error('code table m00100 error：', err)
      }
    ...
    handleCheckInput() {
      ...
      if (!m00137) {
        showModal('请输入学校名称及专业')
        return false
      }
      if (!m00106) {
        showModal('请选择或输入就读学校地址')
        return false
      }
      if (!m00054) {
