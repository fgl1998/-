  ...
  export default {
    ...
    methods: {
        ...
        requestWXGetMyFamilyInfo(idCard)
          .then((res) => {
            ...
            if (data && data.FAMILYID) {
              ...
              this.requestGetMyFamilyUser(FAMILYID)
            } else {
              setDefault()
      ...
      requestGetFamilyDynamic() {
        ...
        requestDSGetDynamicByFamily(idCardList, 1, 3)
          .then((res) => {
            const { list } = res
            if (list) {
              const newList = []
      ...
      // 获取家庭成员
      requestGetMyFamilyUser(familyId) {
        const setDefault = () => {
      ...
      requestGetChi031ByFamily() {
        const idCardList = this.idCardList
        requestDSGetChi031ByFamily(idCardList)
          .then((res) => {
            const { list } = res
            if (list) {
              const newList = []
