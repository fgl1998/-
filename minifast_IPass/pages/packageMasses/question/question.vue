<template>
  <view class="question h5-page">
    ...
    <div class="button-wrapper d-f ai-c jc-sb">
      <ComponentButton name="查询" width="200rpx" height="64rpx" type="primary" @click="handleQuery"></ComponentButton>
      <ComponentButton
        name="我的留言"
        width="200rpx"
        height="64rpx"
        type="primary"
        @click="handleJumpRoute('my-question', 'packageMasses')"></ComponentButton>
    </div>
      ...
      <block v-if="queryResultList.length > 0">
        <div
          ...
          <div class="tags d-f ai-c">
            <div class="tag red">{{ item.chb557_desc }}</div>
            <div class="tag violet" v-show="item.chb55a === '1'">置顶</div>
          </div>
    ...
    <ComponentMUIPopup ref="popup" type="center">
      <div class="popup-content">
        ...
        <div class="cell-wrapper">
          <div class="cell d-f ai-c">
            ...
            <div class="value">
              <picker
                class="picker"
                :range="questionRange"
                @change="handlePickerChangeMessageType"
                :index="messageTypeIndex"
                range-key="name">
                <div class="picker-value" v-if="messageType">
                  {{ questionRange[messageTypeIndex]['name'] }}
                </div>
                <div class="picker-placeholder" v-else>请选择问题类型</div>
                <div class="iconimg iconimg-arrow-right"></div>
              </picker>
          ...
          <div class="cell d-f ai-c">
            <div class="label">咨询区县</div>
            <div class="value">
              <picker
                class="picker"
                :range="districtRange"
                @change="handlePickerChangeDistrict"
                :index="districtIndex"
                range-key="orgname">
                <div class="picker-value" v-if="district">
                  {{ districtRange[districtIndex]['orgname'] }}
                </div>
          ...
          <div class="cell d-f ai-c">
            <div class="label">咨询业务局</div>
            <div class="value">
              <picker
                class="picker"
                :range="unitRange"
                @change="handlePickerChangeUnit"
                :index="unitIndex"
                range-key="aaa103">
                <div class="picker-value" v-if="unit">
                  {{ unitRange[unitIndex]['aaa103'] }}
                </div>
          ...
          <div class="cell d-f ai-c">
            ...
            <div class="value">
              <picker
                class="picker"
                :range="publicRange"
                @change="handlePickerChangePublic"
                :index="publicIndex"
                range-key="aaa103">
                <div class="picker-value" v-if="publicValue">
                  {{ publicRange[publicIndex]['aaa103'] }}
                </div>
                <div class="picker-placeholder" v-else>请选择是否公开</div>
                <div class="iconimg iconimg-arrow-right"></div>
              </picker>
            </div>
          </div>
    ...
    <block>
      <ComponentAffix
        ...
        label="我要留言"
        bottom="162rpx"
        @click="handleQuestion"></ComponentAffix>
    </block>
  </view>
</template>
...
<script>
  ...
  import {
    requestSPAddMsgViewTime,
    requestSPQueryLevMsg,
    requestSPSaveHb55,
  ...
  export default {
    ...
    methods: {
      ...
      async getCollection() {
        // 业务局编码
        const chi037 = await getCodeListByCodeName('chi037')
        const yesorno = await getCodeListByCodeName('yesorno')
        this.codeMap = {
          chi037,
          yesorno
        }
        this.unitRange = [...chi037]
        this.publicRange = [...yesorno]
      },
      // 获取区县range
      getDistrictRange() {
      ...
      handleQuery() {
        ...
        requestSPQueryLevMsg('', questionType, '2', pageNo, pageLimit).then((res) => {
      ...
      // 留言类型 picker change
      handlePickerChangeMessageType(e) {
        const { value } = e.target
        const questionRange = this.questionRange
        const { chi037 } = this.codeMap
        this.messageTypeIndex = value
        this.messageType = questionRange[value].value

        if (value === '2') {
          const newUnitRange = []
          chi037.forEach((item) => {
            if (item.aaa102 === '13') {
              newUnitRange.push(item)
            }
          })
          this.unitRange = [...newUnitRange]
        } else {
          this.unitRange = [...chi037]
        }
      },
      // 咨询业务局 picker change
      handlePickerChangeUnit(e) {
        const { value } = e.target
        const unitRange = this.unitRange
        this.unitIndex = value
        this.unit = unitRange[value].aaa102
      },
      // 咨询区县
      handlePickerChangeDistrict(e) {
        const { value } = e.target
        const districtRange = this.districtRange
        this.districtIndex = value
        this.district = districtRange[value].orgid
      },
      // 是否公开 picker change
      handlePickerChangePublic(e) {
        const { value } = e.target
        const publicRange = this.publicRange
        this.publicIndex = value
        this.publicValue = publicRange[value].aaa102
      },
      // 加载更多
      handleClickLoadMore() {
        this.pageNo++
      ...
      handleSubmit() {
        ...
        const publicValue = this.publicValue
        const { userName, loginId, tel } = this.userInfo
        if (!messageType) {
          showModal('请选择留言类型')
          return
        }
        ...
        requestSPSaveHb55(userName, loginId, unit, messageType, publicValue, messageContent, district, tel).then(() => {
          this.closePopup()
          showModal('留言成功，感谢您的积极参与，稍后会有工作人员对留言进行审核和回复，期待能给您一个满意答复!')
        })
      },
      // 微信留言次数增加
  ...
  .question {
    ...
    .query-result {
      ...
      .see-reply {
        ...
        line-height: 80rpx;
        color: #3882ff;
        border-top: 1px solid rgba(#3882ff, 0.06);
        span {
          margin-right: 10rpx;
        }
        .iconimg {
