<template>
  <div class="sunshine h5-page page-wrapper">
    ...
    <div class="overview-wrapper card-wrapper">
      <ComponentPanel :title="userInfo.aac021 || '-- -- --'" :level="2">
        <div class="panel-content">
          <div class="picker-wrapper d-f ai-c">
            <ComponentPicker
              v-model="issueNumber"
              :range="issueNumberRange"
              rangeKey="name"
              valueKey="code"
              ...
              <div
                ...
                @click="
                  handleJumpRoute('subsidy', 'packageDetails', {
                    ...
                    chb015: userInfo.chb015,
                    chb017: userInfo.chb017,
                    aae209: issueNumber,
                    type: item.type
                  })
                ">
    ...
    <div class="news-wrapper">
      <ComponentNews :renderData="newsData"></ComponentNews>
    </div>

    <!-- 社保卡情况 -->
    <div class="SSC-wrapper" v-if="showSSCPanel">
      <ComponentPanel
        title="社保卡情况"
        buttonText="查看详情"
        @click="
              ...
              <block v-for="(item, index) in subsidyRank" :key="item.id">
                <div class="progress-bar-item">
                  ...
                  <div class="name d-f">
                    ...
                    <div class="label">{{ item.chi031_desc || '----' }}</div>
                  </div>
                </div>
              </block>
            </div>
    ...
    <!-- 了解更多 二维码 -->
    <ComponentQR top="100rpx"></ComponentQR>
  </div>
</template>
<script>
  import imageFollow from './images/follow.png'
  import imageHeader from './images/header.png'
  import imageProcess from './images/process.png'
  import imageRate from './images/rate.png'
  import imageShareText from './images/share-text.png'
  import imageShare from './images/share.png'
  ...
  import {
    ...
    requestFFGetCountOfVillageByChb018,
    requestFFGetDynamicByChb018,
    requestFFGetAae209ListByChb018,
    requestFFGetVillageCountInfo,
    requestFFGetSubsidyRanking,
    requestFFGetVillageDetailInfo
  } from '@/service/api'

  import { M_UPDATE_USER_INFO, USER_INFO, M_UPDATE_USER_LIST } from '@/store/constants/index' // 常量
  ...
  export default {
    ...
    methods: {
      ...
      requestSavaSzvisit(data) {
        ...
        requestB109(idCard, userName, chb015, chb017, chb018, '三资', '村村响', false)
          .then(() => {})
          .finally(() => {})
      },
      // 初始化数据 第一次进入 用户有村庄数据
      async initData() {
        showLoading()
        const { chb015, chb017, chb018 } = this.userInfo
        // 存在 地址信息
        if (chb015 && chb017 && chb018) {
          Promise.all([
            this.requestGetCountOfVillageByChb018(chb015, chb017, chb018, false),
            this.requestGetDynamicByChb018(chb015, chb017, chb018, false),
            this.requestGetAae209ListByChb018(chb015, chb017, chb018, false)
          ]).finally(() => {
            hideLoading()
          })
        } else {
          hideLoading()
          showModal('查询过程出错，请联系‘宣汉三资’技术人员解决')
        }
      ...
      getH5UserInfo() {
        requestWXGetUserInfoByDzsb()
          .then((res) => {
            const { aac002, aac003, chb015, chb017, chb018, channelNo } = res.data
            const data = {
              ...res.data,
              userName: aac003,
              idCard: aac002,
      ...
      // 请求 查询村庄关注人数
      requestGetCountOfVillageByChb018(chb015, chb017, chb018, isLoading = true) {
        return new Promise((resolve, reject) => {
          requestFFGetCountOfVillageByChb018(chb015, chb017, chb018, isLoading, false)
            .then((res) => {
              const { fieldData } = res
              if (fieldData) {
                const { aac002_count } = fieldData
                this.$store.commit(M_UPDATE_USER_INFO, {
                  aac002_count
                })
                resolve(aac002_count)
              } else {
                this.$store.commit(M_UPDATE_USER_INFO, {
                  aac002_count: 0
                })
      ...
      // 获取最新动态
      requestGetDynamicByChb018(chb015, chb017, chb018, isLoading = true) {
        return new Promise((resolve, reject) => {
          requestFFGetDynamicByChb018(chb015, chb017, chb018, isLoading, false)
            .then((res) => {
              const { list } = res
              if (list) {
                this.newsData = [...list]
            ...
            .catch((err) => {
              showModal(err.message || '最新动态查询失败！')
              reject(err)
            })
        })
      },
      ...
      // 获取一个村申报过的期号
      requestGetAae209ListByChb018(chb015, chb017, chb018, isLoading = true) {
        this.issueNumberRange = []
        return new Promise((resolve, reject) => {
          requestFFGetAae209ListByChb018(chb015, chb017, chb018, isLoading, false)
            .then((res) => {
              const arr = []
              const { list } = res
              if (list) {
                ...
                this.issueNumberRange = [...arr]
                const issueNumber = arr[0].aae209
                this.issueNumber = issueNumber
              }
              resolve(res)
            })
            .catch((err) => {
              showModal(err.message || '未查询到该村（社区）申报的补贴数据')
              reject(err)
            })
        })
      },
      ...
      // 村村响概况查询
      requestGetVillageCountInfo(aae209, isLoading = true) {
        const { chb018, chb017, chb015 } = this.userInfo

        requestFFGetVillageCountInfo(aae209, chb017, chb018, chb015, isLoading, false)
          .then((res) => {
            const { fieldData } = res
            this.villageInfo = {
              ...fieldData,
          ...
          .catch((err) => {
            showModal(err.message || '统计数据查询失败')
          })
      },

      // 补贴项目概况
      requestGetSubsidyRanking(aae209, isLoading = true) {
        this.chi031 = '' // 解决不同期号相同chi031不请求数据
        const { chb018, chb017, chb015 } = this.userInfo

        requestFFGetSubsidyRanking(aae209, chb017, chb018, chb015, isLoading, false)
          .then((res) => {
            const { list } = res
          ...
          .catch((err) => {
            ...
            this.subsidyRank = []
            this.pageNo = 1
            showModal(err.message || '该补贴项目没有更多数据')
          })
      },

      // 补贴明细查询
      requestGetVillageDetailInfo(chi031, isLoading = true) {
        const { chb018, chb017, chb015 } = this.userInfo
        ...
        requestFFGetVillageDetailInfo(
          ...
          .catch((err) => {
            showModal(err.message || '查询补贴信息出错')
          })
      },
      ...
      handleClickLoadMore() {
        this.pageNo++
        this.requestGetVillageDetailInfo(this.chi031)
      },
    ...
    watch: {
      ...
      chi031: {
        handler(val) {
          if (val) {
            ...
            this.isLoadOver = true
            this.isExceedLimit = false
            this.requestGetVillageDetailInfo(val)
          }
        },
        immediate: true
      },
      ...
      issueNumber: {
        handler(val) {
          if (val) {
            this.requestGetVillageCountInfo(val)
            this.requestGetSubsidyRanking(val)
          }
        },
        immediate: true
      },
