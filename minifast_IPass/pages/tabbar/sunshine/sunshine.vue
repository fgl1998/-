  ...
  <div class="sunshine h5-page page-wrapper">
    ...
    <div class="overview-wrapper card-wrapper">
      <ComponentPanel
        ...
        :buttonText="userInfo.aac021 ? '切换地址' : '选择地址'"
        :level="2"
...
<script>
  ...
  import { navigateTo, showModal, showLoading, hideLoading } from '@/utils/uni-api/index'
  import {
    requestFFSavePersonalInfo,
  ...
  export default {
    ...
    methods: {
      ...
      async initData() {
        ...
        const { chb017, chb018, user_id, openid } = this.userInfo
        if (isRequestTown) {
          // 存在 乡镇信息
          if (chb017) {
            Promise.all([
              this.getVillageNum(chb017, false),
              this.getNewsData(chb017, false),
              this.getIssueNumberRange(chb017, false)
            ]).finally(() => {
              hideLoading()
            })
          } else {
            ...
            if (!config.isShowShutDownTips) {
              showModal('未获取到个人相关乡镇街道信息,请先去完善！').then((res) => {
          ...
          if (chb018) {
            // 设置选择地址弹窗的初始地址
            // if (aac021.indexOf('-') > -1) {
            //   this.address = aac021.split('-')
            // }
            Promise.all([
              this.getVillageNum(chb018, false),
              this.getNewsData(chb018, false),
              this.getIssueNumberRange(chb018, false)
            ]).finally(() => {
              hideLoading()
            })
          } else {
      ...
      // 请求 查询村庄关注人数
      getVillageNum(chb018, isLoading = true) {
        const isRequestTown = this.isRequestTown
        let { chb015 } = this.userInfo
        let chb017 = ''
        if (isRequestTown) {
          chb017 = chb018
          chb018 = ''
      ...
      // 获取最新动态
      getNewsData(chb018, isLoading = true) {
        const isRequestTown = this.isRequestTown
        let { chb015 } = this.userInfo
        let chb017 = ''
      ...
      // 获取一个村申报过的期号
      getIssueNumberRange(chb018, isLoading = true) {
        const isRequestTown = this.isRequestTown
        let { chb015 } = this.userInfo
        let chb017 = ''
        if (isRequestTown) {
      ...
      // 村村响概况查询
      getVillageInfo(aae209, isLoading = true) {
        const isRequestTown = this.isRequestTown
        let { chb018, chb017, chb015 } = this.userInfo
        if (isRequestTown) {
          chb018 = ''
        } else {
          chb017 = ''
        ...
        requestFFGetVillageCountInfo(aae209, chb017, chb018, chb015, isLoading, false)
          ...
          .catch((err) => {
            showModal(err.message || '统计数据查询失败', '提示', false)
          })
      },
      // 补贴项目概况
      getSubsidyRanking(aae209, isLoading = true) {
        ...
        requestFFGetSubsidyRanking(aae209, chb017, chb018, chb015, isLoading, false)
          ...
          .catch((err) => {
            ...
            this.subsidyRank = []
            this.pageNo = 1
            showModal(err.message || '该补贴项目没有更多数据', '提示', false)
          })
      },
      // 补贴明细查询
      getSubsidyList(chi031, isLoading = true) {
        ...
        const isRequestTown = this.isRequestTown
        let { chb018, chb017 } = this.userInfo
        if (isRequestTown) {
          chb018 = ''
        } else {
          chb017 = ''
      ...
      handleChangeAddress(address, addressInfo) {
        ...
        if (addressInfo[1]) {
          ...
          if (this.areaCode === '510400') {
            chb017 = addressInfo[2].orgid
          }
        }
        if (addressInfo[2]) {
          chb018 = addressInfo[2].orgid // 村编码
          if (this.areaCode === '510400') {
            chb018 = addressInfo[3].orgid
          }
        }
        ...
        if (aac021 && chb015) {
          requestFFSavePersonalInfo(data, false).then(() => {
            this.$store.commit(M_UPDATE_USER_INFO, {
              aac021,
              chb015,
              chb017,
      ...
      handleJumpRouteComplaint(item) {
        ...
        // 兼容巴中
        if (areaCode == '511900') {
          navigateTo('complaints-bz', 'packageDetails', item)
        } else {
    ...
    watch: {
      ...
      issueNumber: {
        handler(val) {
          if (val) {
            this.getVillageInfo(val)
            this.getSubsidyRanking(val)
          }
        },
        immediate: true
      },
      ...
      'userInfo.chb018': {
        handler(val) {
          ...
          if (this.isRequestTown) {
            return
          }
          this.requestParamsAddress = config.FFAddressRequestParams
          this.issueNumber = ''
          this.initData()
        },
        immediate: true
