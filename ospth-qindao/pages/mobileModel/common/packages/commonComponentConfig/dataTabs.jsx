export default {
  ...
  created () {
    // 初始化校验
    if (!this.config.initUrl || this.config.initUrl?.length === 0) this.urlError = true
  ...
  methods: {
    // url校验
    fnOnUrlBlur () {
      if (!this.config.initUrl || this.config.initUrl?.length === 0) {
    ...
    getCollectionData(){
      if(this.config.collectionType) {
        Base.asyncGetCodeData(this.config.collectionType).then((codeList) => {
          let collectionData = Base.doCodeFilter(codeList, this.config.collectionFilter, this.config.reverseFilter)
          this.config.collectionData = collectionData.map(item => {return {name: item.value, label: item.label}})
        })
      }
    }
  },
      ...
      <ta-tabs
        ...
        {this.tabsName.includes('dict') && <ta-tab-pane tab={'字典数据'} key={'dict'}>
          <ta-label-con label={'字典类型'}>
            <ta-input vModel_trim={config.collectionType} vOn:blur={this.getCollectionData} placeholder="请输入字典类型"/>
          </ta-label-con>
          <ta-label-con label="过滤">
            <ta-input vModel_trim={config.collectionFilter} vOn:blur={this.getCollectionData} placeholder="请输入过滤字典键值，逗号隔开"/>
          </ta-label-con>
          <ta-label-con label="反向过滤" style="margin-bottom: 0">
            <ta-switch vModel={config.reverseFilter} vOn:change={this.getCollectionData}/>
          </ta-label-con>
        </ta-tab-pane>}
        {/* 远程数据tab页 */}
