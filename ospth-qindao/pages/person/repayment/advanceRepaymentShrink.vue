<template>
  ...
  <common-layout :title="title" :leftArrow="true" :clickLeft="clickLeft">
    <sel-loan-acc  v-if="stepNum === 0" :startInit="stepNum === 0"
       :agreementTypeArr="['TQHKSQTSZHDK']" @next="val => { chooseLoanInfo = val; stepNum++ }"/>
    <pay-info v-if="stepNum === 1" :startInit="stepNum === 1"
      ...
      <template slot-scope="{info,calInfo}">
        ...
        <van-collapse accordion v-model="collapseValue">
          <van-collapse-item :name="1" title="查看还款计划变更">
            ...
            <van-cell title="还款后还款期数：" :value="calInfo.changeAftMonthPayWay"></van-cell>
...
export default {
  ...
  methods: {
    ...
    async completed () {
      ...
      //贷款业务号
      params.loanBusinessNo = this.chooseLoanInfo.loanBusinessNo
