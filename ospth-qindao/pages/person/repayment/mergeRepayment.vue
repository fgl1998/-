<template>
  ...
  >
    ...
    <pay-info
      ...
      ref="payInfo"
      :startInit="stepNum === 1"
      :needInpMergeRepayBank="true"
      @next="payInfoNext"
    >
      <template slot-scope="{info,calInfo}">
        <van-cell
          title="提前偿还利息(元)："
          :value="info.advancePaymentRateMny | moneyFormat"
        />
        <van-cell
          title="本次还款合计(元)："
          :value="info.loanRepaymentAmount | moneyFormat"
        />
        <van-collapse
          accordion
        ...
        >
          ...
          >
            <van-cell
              title="还款后下月应还本金(元)："
              :value="calInfo.nextMonthRepaymentPrincipal | moneyFormat"
            />
            <van-cell
              title="还款后下月应还利息(元)："
              :value="calInfo.nextMonthRateMny | moneyFormat"
            />
            <van-cell
              title="还款后下月应还总额(元)："
              :value="calInfo.nextMonthRepaymentTotal | moneyFormat"
            />
            <van-cell
              title="还款后还款期数："
              :value="calInfo.repaymentAftLoanMonth"
            />
    ...
    <hf-phone-or-face-auth
      v-if="stepNum === 2"
      :commonBizApply="commonBizApply"
      @cancel="stepNum--"
      @nextStep="completed"
    />
    <hf-biz-evaluation
...
export default {
  ...
  data () {
    return {
      ...
      collapseValue: '',
      payAccountInfo: { mergeRepayGjj: 0 },
    }
  },
  computed: {
