import { APP_NAME, IMAGE_REPLACE_DIRECTORY, BASE_IP, BASE_URL, IMAGE_SRC, DEFAULT_AREA_CODE, DEFAULT_AREA_NAME, OSPTP_NAME, } from '@/config.js'
 ...
 * @onFailure 失败回调
 */
function request(url, params, method, header, onSuccess, onFailure, showToast = true) {
	if (url.indexOf('http') == -1) {
		url = BASE_URL + url
	}

	let loading;
	if (showToast) loading = setTimeout(function() {
		uni.showLoading({
			mask: true,
			title: '加载中'
		});
		...
		success: function(res) {
			...
			if (res.data.serviceSuccess) {
				...
				if (res.data.message == TOKEN_LOSE) {
					uni.removeStorageSync("user")
					setToken(res.data.resultData.data)
					uni.showModal({
						title: '提示',
						content: '网络错误，请重试'
					});
					...
					if (onFailure) {
						...
						uni.showModal({
							...
							showCancel: false,
							success: () => {
								if (message && (message.indexOf("登录信息已失效") != -1 || message
										.indexOf('用户未登录') != -1 || message.indexOf(
											'请先登录后再进行该操作') != -1)) {
									// 判断是否为小程序环境
									if (isWechat()) {
...
function queryImg(idCard, img_cj, onSuccess, onFailure, isFirst) {
	...
	param.operator = "达州人社";
	post('/custom/auth/queryCardImg.do', param, data => {
...
function checkInfo(img_cj, img_Sb, idCard, onSuccess, onFailure) {
	...
	param.image1 = img_cj;
	param.image2 = img_Sb;
	post('/custom/AppImageCompare/compare.do', param, data => {
...
function decryptSocial(security, onSuccess, onFailure) {
	post('/social/decryptHtmlSign.do', {
		'security': security
	}, data => {
...
function checkServiceAuth(param) {
	...
	post('/auth/checkAuth', params, res => {
		if (res.code == '-1') {
			//校验失败
			uni.showModal({
				title: '提示',
