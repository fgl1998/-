...
function appScssVar(oldVarString) {
  const newData = Object.keys(styleVariables)
    .filter((key) => key == 'image_replace_directory' || key == 'image_src')
...
module.exports = {
  ...
  configureWebpack: {
    ...
    resolve: {
      alias: {
        ...
        '@osppm': path.resolve(__dirname, '../osppm'),
        '@api-request': path.resolve(__dirname, './common/psmp/js/request.js'),
      },
      symlinks: false,
    },
    module: {
  ...
  chainWebpack: (webpackConfig) => {
    // 确保 @api-request 被正确解析（osppm/api/request.js 依赖），避免差量编译等场景下 resolve 失效
    webpackConfig.resolve.alias.set('@api-request', path.resolve(__dirname, './common/psmp/js/request.js'))

    webpackConfig.module.rule('mjs$')
