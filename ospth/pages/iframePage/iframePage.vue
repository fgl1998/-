<template>
  <view class="pages">
    ...
    <view class="main" v-show="stepKeys[current1 - 1] == 'step1'">
      ...
      <view class="btnBox">
        <view class="setBtn">
          <van-button type="default" class="btnStyle" v-if="reportList.length > 0" @click="fnToPrint()"
            >打&nbsp;&nbsp;&nbsp;&nbsp;印</van-button
          >
        ...
        <view class="btnBox">
          <view class="setBtn">
            <van-button
              type="default"
              class="btnStyle"
              v-if="showPre || serviceTip || businessSceneVos.length > 1"
              @click="fnToPre()"
              >上一步</van-button
            >
            <van-button
              ...
              class="btnStyle"
              @click="fnToValidata()"
              :disabled="disableNext || (pageEcho == '1' && materialVosCopy.length == 0)"
              >{{ materialVosCopy.length > 0 ? '下一步' : '提&nbsp;&nbsp;&nbsp;&nbsp;交' }}</van-button
            >
          </view>
        </view>
      </view>
    ...
    <view v-if="stepKeys[current1 - 1] == 'step3'" class="mt12">
      ...
      <view class="uploadBox">
        <upload
          :materialList="materialVosCopy"
          :taskBasicGuid="taskBasicGuid"
          :projectNo="projectNo"
      ...
      <view class="btnBox">
        <view class="setBtn">
          ...
          <van-button type="info" class="btnStyle" @click="fnToSave()" :disabled="pageEcho == '1'"
            >提&nbsp;&nbsp;&nbsp;&nbsp;交</van-button
          >
        </view>
      </view>
          ...
          <view class="tips_content" style="text-align: center">
            <p style="width: 80%; margin: 0 auto">
              <span style="font-family: 'Arial Normal', 'Arial'; font-weight: 400"
                >您的本次办件的业务统一办件编号为</span
              >
              <span style="font-family: 'Arial Normal', 'Arial'; font-weight: 400; color: #f59a23"></span>
              <span style="font-family: 'Arial Negreta', 'Arial Normal', 'Arial'; font-weight: 700; color: #f59a23">{{
                projectNo
              }}</span>
              <span style="font-family: 'Arial Normal', 'Arial'; font-weight: 400"
                >，后续可通过办件号在公共服务平台各渠道上进行自助查询，也可在各渠道的【我的- 我的办件】进行查询。</span
              >
            </p>
          </view>
        </view>
    ...
    <van-popup v-model="showDialog" round class="showDialog">
      <view class="title">提示</view>
      <view class="dialog">办理结果是否需要邮寄，若不需要邮寄则需要前往线下大厅自取？</view>
      <view class="btnBox showLine">
        <view class="btn" @click="fnPost(false)">自取</view>
        <view class="btn comfirm" @click="fnPost(true)">邮寄</view>
      </view>
    </van-popup>

    <van-popup v-model="showPost" round class="showDialog">
      <view class="title">邮寄信息</view>
      <view class="post">
        <van-cell-group>
          <van-field v-model="postInfo.name" label="姓名" placeholder="请输入" />
          <van-field v-model="postInfo.phoneNum" label="手机号" placeholder="请输入" />
          <van-field v-model="postInfo.address" label="邮寄地址" placeholder="请输入" />
        </van-cell-group>
      </view>
      <view class="btnBox">
        <view class="btn" @click="showPost = false">取消</view>
        <view class="btn comfirm" @click="fnConfirmPost">确认</view>
      </view>
    </van-popup>

    <van-popup v-model="showReason" round class="showDialog">
      <view class="title">撤销原因</view>
      <view class="post">
        <van-cell-group>
          <van-field v-model="reason" placeholder="请输入" />
        </van-cell-group>
      </view>
      <view class="btnBox">
        <view class="btn" @click="showReason = false">取消</view>
        <view class="btn comfirm" @click="fnToCancle">确认</view>
      </view>
    </van-popup>
  </view>
</template>
...
export default {
  ...
  onLoad(option) {
    ...
    let channelCode = option.channelCode
    uni.setStorageSync('channelCode', channelCode == undefined ? 'osp-02-006-app' : channelCode)
    this.serviceUrl = this.$base.BASE_IP + '/template/h5/index.html#/pages/jumpPage/jumpPage'
    uni.setNavigationBarTitle({
      title: this.handleItemName,
    })
    uni.setStorageSync('service_handleItemName', this.handleItemName)
    ...
    new Promise((resolve, reject) => {
      ...
      this.$base.post(
        ...
        (res) => {
          if (res.taskBasicVo) {
            let cleanData = JSON.parse(JSON.stringify(res.taskBasicVo))
            this.service = cleanData
            this.projectNo = res.projectNo
            console.log('res.user: ', res.user)
            uni.setStorageSync('user', res.user)
            console.log('res.user1111===============-----------: ', res.user)
  ...
  methods: {
    ...
    initWebsocket() {
      ...
      uni.connectSocket({
        url: this_.$base.BASE_URL.replace('http', 'ws') + '/websocket/h5_' + this.user.accountBo.accountId,
      })
    ...
    fnInit() {
      ...
      if (this.checkForm()) {
        ...
        this.serviceUrl = this.serviceUrl.replace('8082', '8087')
        //低代码表单进入特定页面
        if (formVo && formVo.formType == '1') {
          this.serviceUrl =
            ...
            '/template/h5/index.html#/pages/mobileModel/mobileModel?templateCode=' +
            formVo.formCode
        }
      }
      ...
      if (moreInfo) {
        ...
        if (moreInfo.serveType != '' && moreInfo.serveType != null && typeof moreInfo.serveType != 'undefined') {
          this.serverType = Number(moreInfo.serveType)
        }
      }
      if (
        this.handleItemCode &&
        this.handleItemCode.substring(25, 26) != '0' &&
        this.handleItemCode.substring(25, 26) != ''
      ) {
        this.serverType = 2
      }
      ...
      if (
        (this.serviceTip == '' || this.serviceTip == null || typeof this.serviceTip == 'undefined') &&
        this.businessSceneVos.length <= 1
      ) {
        this.fnToIframe()
      }
    },
    ...
    fnSceneChange(checkedValues) {
      ...
      this.materialVos.forEach((v) => {
        // 无情形、包含当前情形、包含共有情形
        if (
          (!v.sceneCode || v.sceneCode.indexOf(this.sceneCode) !== -1 || v.sceneCode.indexOf('S1111') !== -1) &&
          v.channelType.indexOf(uni.getStorageSync('channelCode')) !== -1
        ) {
          let total = 0
          if (v.originalPageNum) {
            total += Number(v.originalPageNum)
    ...
    fnGetTicket() {
      ...
      this.$base.post('/h5/getTicket', param, (res) => {
        if (res.data) {
          ...
          let paramStr =
            'token=' +
            encodeURIComponent(uni.getStorageSync('applet-token')) +
            '&handleItemCode=' +
            this.handleItemCode +
            '&areaCode=' +
            this.areaCode +
            '&old=' +
            uni.getStorageSync('old') +
            '&channelCode=' +
            this.getChannelCode() +
            '&handleItemName=' +
            encodeURIComponent(this.handleItemName) +
            '&ticket=' +
            encodeURIComponent(res.data) +
            '&socialNo=' +
            encodeURIComponent('650542626') +
            '&projectNo=' +
            encodeURIComponent(this.projectNo) +
            '&moreInfo=' +
            encodeURIComponent(this.service.taskBasicFormVos[0])
    ...
    fnYwsqfw() {
      ...
      if (this.user && this.user.userType) {
        ...
        recer = loginUser.userType == 'UT02' ? loginUser.legalBo.legalRepresentativeName : loginUser.naturalBo.name
        conerMob =
          loginUser.userType == 'UT02' ? loginUser.legalBo.legalRepresentativeMobile : loginUser.naturalBo.mobile
        cardCode =
          loginUser.userType == 'UT02' ? loginUser.legalBo.legalRepresentativeCertNo : loginUser.naturalBo.certNo

        applyerType = this.user.userType == 'UT02' ? '01' : '02'
        appyerCode = this.user.accountBo.accountId
        appyerCardCode = this.user.userType == 'UT02' ? this.user.legalBo.legalCertNo : this.user.naturalBo.certNo
        appyerName = this.user.userType == 'UT02' ? this.user.legalBo.legalName : this.user.naturalBo.name
        appyerCardType = this.user.userType == 'UT02' ? this.user.legalBo.legalCertType : this.user.naturalBo.certType
      }
      ...
      let bmcGeRecevieDTO = {
        ...
        channelType: channelCode == 'osp-02-007-wechat' ? '小程序' : channelCode == 'osp-02-006-app' ? 'app' : '移动端', //服务渠道名称（综柜、网厅等等）
    ...
    getBmcMaterialDtos(appyerCardCode, appyerName) {
      ...
      for (let i in this.materialVosCopy) {
        let sinFlag =
          (this.materialVosCopy[i].needPaperMaterial == '1' && this.materialVosCopy[i].needEMaterial == '1') == true
            ? '1'
            : '0' //需要纸质材料（needPaperMaterial）和 需要电子材料（需要电子材料）
        let paperCount = 0
