...
export default {
  name: 'cascader-component-config',
  ...
  methods: {
    addTreeChildren(data, formData) {
      data.forEach(item => {
        if (item.value === this.nodeData.value) {
          if(!item.children) {
            this.$set(item, 'children', [formData])
          } else {
            item.children.push(formData);
          }
        } else {
          if (item.children) {
            this.addTreeChildren(item.children, formData);
          }
        }
      });
    },
      ...
      this.form.validateFieldsAndScroll((error) => {
        if (!error) {
          ...
          if (this.operateType === 'add') {
            ...
            this.addTreeChildren(this.config.staticData, formData)
          } else if (this.operateType === 'edit') {
            const value = this.nodeData.value
            if (this.treeAllKeys.includes(formData.value)&&formData.value!=this.nodeData.value) {
              this.$message.error('节点value重复')
              return
            }
            this.treeAllKeys.splice(this.treeAllKeys.indexOf(value),1);
            this.$set(this.nodeData, 'text', formData.text)
  ...
  render () {
    return (
        ...
        >
          <ta-label-con label="标题栏">
            <ta-switch vModel={this.config.showHeader}/>
          </ta-label-con>
          {
              ...
              <div>
                ...
                <ta-label-con label="关闭图标">
                  <ta-switch vModel={this.config.closeable}/>
                </ta-label-con>
              </div>
          }
          <div style={{marginBottom: '24px'}}>
            ...
            <ta-modal
              ...
              <ta-form autoFormCreate={(form) => {
                ...
                <ta-form-item
                    label="label"
                    fieldDecoratorId="text"
                    require={{message: '请输入节点label'}}
                    initValue={this.operateType === 'edit' ? this.nodeData.text : ''}>
                  <ta-input/>
                </ta-form-item>
              </ta-form>
            </ta-modal>
