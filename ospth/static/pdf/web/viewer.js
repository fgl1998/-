...
var PDFViewerApplication = {
  ...
  download: function download() {
    ...
    if(url.indexOf("link=1") != -1) {
      document.getElementById("urlLinks").value = url;
      document.getElementById("shareModel").style.setProperty("display", "block", "important");
      return;
    }
    try {
      //判断如果是微信浏览器则弹出提示
      ...
      if(ua.match(/MicroMessenger/i) == "micromessenger") {
        var weChatAlert = document.getElementById("weChatAlert");
        weChatAlert.style.setProperty("display", "block", "important");
        document.getElementById("weChatAlert").addEventListener("click", function () {
          weChatAlert.style.setProperty("display", "none", "important");
        });
        return;
      }
    } catch (e) {}
    if(url.indexOf("type=mobile") != -1) {
      //页面创建a标签，拼接url并自动点击下载
      var down = document.createElement("a");
      down.download = '';//下载文件名,默认原文件名
      if(url.indexOf("?") != -1) {
        down.href = url+"&download=true";
      } else {
        down.href = url+"?download=true";
      }
      document.body.appendChild(down); //将元素放在页面上
      down.click();
      down.remove();
      return;
    }
...
document.webL10n = function (window, document, undefined) {
  ...
  function parseResource(href, lang, successCallback, failureCallback) {
    ...
    function evalString(text) {
      ...
      let decodedText = JSON.parse(`"${text.replace(/"/g, '\\"')}"`);
